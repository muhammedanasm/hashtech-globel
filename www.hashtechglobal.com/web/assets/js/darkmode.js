...
function darkMode() {
  ...
  function dark_mode_classes(txt, cls) {
    document
      ...
      .forEach((item) => item.classList.toggle(cls));
  }
  ...
  dark_mode_classes(".cnt-btn", "dark-btn-cnt"); // textarea background color
  ...
  dark_mode_class(".newsletter-inpt", "newsletter-inpt-dark"); // Inner Page Blog Date text
  ...
  dark_mode_classes(".cnvs-icn", "social-color-dark"); //Social Icon Dark
  ...
  document
    .querySelectorAll(".light-logo")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".dark-logo")
    .forEach((item) => item.classList.toggle("d-none"));

  // Client Image Change

  document
    .querySelectorAll(".client-light-img")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".client-dark-img")
    .forEach((item) => item.classList.toggle("d-none"));

  // Product Mentagoz Logo Change

  document
    .querySelectorAll(".product-ment-logo")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".product-ment-logo-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // drawsvg Mentagoz Logo Change

  document
    .querySelectorAll(".mentlogosvg")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".mentlogosvg-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // world Mentagoz Logo Change

  document
    .querySelectorAll(".ment-logo")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".ment-logo-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // Service Page Ment Logo change

  document
    .querySelectorAll(".mentservicelogo")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".mentservicelogo-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // Product Page Ment Logo change

  document
    .querySelectorAll(".mentproductlogo")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".mentproductlogo-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // Product Logo change Home Page

  document
    .querySelectorAll(".product-logo")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".product-logo-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // Footer Logo Change

  document
    .querySelectorAll(".footer-logo-img")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".footer-logo-img-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // Divider Change Darkmode

  document
    .querySelectorAll(".top-divider-img")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".top-divider-img-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  document
    .querySelectorAll(".bottom-divider-img")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".bottom-divider-img-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // 3D Element

  document
    .querySelectorAll(".threed_light")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".threed_dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // Client Logo image
  document
    .querySelectorAll(".client-logo-img")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".client-logo-img-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // SVG Progress Change
  document
    .querySelectorAll(".svg-light")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".svg-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // Web Design Inner Banner
  document
    .querySelectorAll(".web-inner-banner-light")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".web-inner-banner-dark")
    .forEach((item) => item.classList.toggle("d-none"));

  // Client Page Ment GIF Dark
  document
    .querySelectorAll(".light-client-gif")
    .forEach((item) => item.classList.toggle("d-none"));
  document
    .querySelectorAll(".dark-client-gif")
    .forEach((item) => item.classList.toggle("d-none"));
